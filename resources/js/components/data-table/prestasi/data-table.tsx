...
import { DialogCreate } from '../../dialog-form/prestasi/dialog-form-create';
import { DialogDelete } from '../../dialog-form/prestasi/dialog-form-delete';
import { DialogEdit } from '../../dialog-form/prestasi/dialog-form-update';
import { Input } from '../../ui/input';

export const schema = z.object({
    id: z.number(),
    nama_prestasi: z.string(),
    jenis_prestasi: z.string(),
...
const createColumns = (handlers: {
    ...
    },
    {
        ...
        cell: ({ row }) => <div className="w-min-32 w-full font-medium">{row.original.nama_prestasi}</div>,
        enableHiding: false,
    },
    {
        accessorKey: 'jenis_prestasi',
        header: 'Jenis Prestasi',
        cell: ({ row }) => <div className="w-min-32 w-full font-medium">{row.original.jenis_prestasi}</div>,
        enableHiding: false,
    },
    {
...
export function DataTable({ data: initialData, auth }: { data: z.infer<typeof schema>[]; auth: any }) {
    ...
    const [selectedJenisPrestasi, setSelectedJenisPrestasi] = React.useState<z.infer<typeof schema>>({
        id: 0,
        nama_prestasi: '',
        jenis_prestasi: '',
        jumlah_laporan: 0,
    ...
    return (
        <Tabs defaultValue="outline" className="flex w-full flex-col justify-start gap-6">
            <div className="flex items-center justify-between gap-2 px-4 lg:px-6">
                ...
                <div className="relative w-full max-w-sm">
                    ...
                    <Input
                        ...
                        value={(table.getColumn('nama_prestasi')?.getFilterValue() as string) ?? ''}
                        onChange={(event) => table.getColumn('nama_prestasi')?.setFilterValue(event.target.value)}
                        className="h-8 pl-7"
                    />
            ...
            <DialogEdit data={selectedJenisPrestasi} open={openDialogUpdate} setOpen={setOpenDialogUpdate} />
            <DialogDelete data={selectedJenisPrestasi} open={openDialogDelete} setOpen={setOpenDialogDelete} />
        </Tabs>
    );
}
